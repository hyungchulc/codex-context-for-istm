...
- Confirm no model packet, model result, model-state cursor, committed model-Daily batch, generated STM card, or apply marker is tracked.
...
- Confirm packaged result schemas remain closed and are included in a clean wheel/install.
...
The audit script searches tracked text for common absolute personal paths, credential-like prefixes, UUID-shaped identifiers, private runtime artifact names, and missing/open top-level result schemas. It cannot reliably detect every secret or sensitive phrase, recursively prove every schema closure, or validate provider behavior, so it must not be treated as proof of safety.
